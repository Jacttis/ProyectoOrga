...
extern tPosicion l_anterior(tLista l, tPosicion p){
    tPosicion posicionActual=l; // posicionActual variable que busca el anterior a p,Empezando desde el primer elemento
...
/*tLista clonar(tLista lista){
    tLista  nueva;
    nueva=(tLista )malloc(sizeof(struct celda));
    crear_lista(&nueva);
    tPosicion actual=lista->siguiente;
    while(actual!=NULL){
        l_insertar(nueva,l_fin(nueva),actual->elemento);
        actual=actual->siguiente!=NULL?actual->siguiente:NULL;
    }
    return nueva;*/
//
